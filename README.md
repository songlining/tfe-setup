...
**Note**: TFE always serves HTTPS internally. The "TLS termination" mode is actually "TLS re-encryption" because TFE's internal nginx will redirect HTTP to HTTPS when accessed via its configured hostname (`TFE_HOSTNAME`).
